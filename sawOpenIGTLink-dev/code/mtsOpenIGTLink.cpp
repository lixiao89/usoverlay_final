...
void mtsOpenIGTLink::Configure(const std::string & hostAndPort)
{
    ...
    mtsInterfaceRequired * requiresPositionCartesion = AddInterfaceRequired("RobotBase");
...
void mtsOpenIGTLink::Run(void)
{
    ...
    std::cout<<NDI.PositionCartesian.Position().Rotation().Element(0,0)<<std::endl;
    ...
    if (IsConnected) {
        ...
        prmPositionCartesianToOIGTL(NDI.PositionCartesian, dataMatrix);
...
void mtsOpenIGTLink::prmPositionCartesianToOIGTL(const prmPositionCartesianGet& frameCISST,
    ...
    frameIGTL[0][3] = frameCISST.Position().Translation().Element(0) - 250;
    frameIGTL[1][3] = frameCISST.Position().Translation().Element(1) - 42.3;
    frameIGTL[2][3] = frameCISST.Position().Translation().Element(2) + 1592;
    frameIGTL[3][3] = 1;
